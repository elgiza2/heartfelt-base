...
for (const [id, provider] of Object.entries(providers)) {
  ...
  apps.push({
    id: `nango:${id}`,
    name: provider.display_name || title(id),
    category,
...
apps.sort((a, b) => Number(b.popular) - Number(a.popular) || a.name.localeCompare(b.name));
...
export const NANGO_APPS: ApiApp[] = ${JSON.stringify(
  apps.map(({ popular, ...app }) => ({ ...app, tools: [] })),
  null,
  2,
)};
`;
...
console.log(`wrote ${apps.length} apps`);
